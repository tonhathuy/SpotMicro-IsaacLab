...
def low_height_penalty(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, min_height: float, scale: float) -> torch.Tensor:
    """Penalize when the robot's base height is too low, indicating a lying-down posture."""
    asset: Articulation = env.scene[asset_cfg.name]
    height = asset.data.root_pos_w[:, 2]  # Chiều cao thân robot so với mặt đất
    penalty = torch.where(height < min_height, scale * (min_height - height), torch.zeros_like(height))
    return penalty
